...
export const voucherDeletePreviewSchema = z.object({
  ...
  allAllocations: z.number().int().nonnegative(),
  historicalTransactions: z.number().int().nonnegative(),
  nonIssueTransactions: z.number().int().nonnegative(),
  deletionMode: z.enum(['permanent', 'reversal']),
