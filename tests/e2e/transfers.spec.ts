...
test('SMK-TRF-001 — transfere estoque entre eventos e exibe o histórico', async () => {
  ...
  try {
    ...
    const sourceOperateButton = sourceEventCard.getByRole('button', { name: 'Operar evento' });
    if (await sourceOperateButton.isVisible()) {
      await sourceOperateButton.click();
    }
    ...
    const destinationOperateButton = destinationEventCard.getByRole('button', {
      name: 'Operar evento',
    });
    if (await destinationOperateButton.isVisible()) {
      await destinationOperateButton.click();
    }
